...
const TestCases = () => {
 ...
 ;   const [questions, setQuestions] = useState({
    ...
    useEffect(() => {
        setLoading(true);
        // console.log("location.state.cases ", location.state.cases);
        // console.log("location.state.error ", location.state.error);
        // setQuestions({
        //     "cases": location.state.cases,
        //     "error": location.state.error
        // });
        setQuestions(state);

        console.log("questions result in useEff: ", questions);
        setLoading(false);
    // }, [])
}, [])
            ...
            <Row className="heightManage">
                ...
                <Col xxl={4} lg={4} md={4} sm={12} xs={12}>
                    <Row className="h-100">
                        ...
                        <div className='flex-col justify-content-end'><Button onClick={goBack} className={"backbtn"}>Go Back</Button></div>
                ...
                <Col xxl={8} lg={8} md={8} sm={12} xs={12} >
                    <Row className="h-100">
                        ...
                        {/* <Card border="light" style={{ width: '18rem' }}> */}
                        <Row className="px-3 makethisscrollable">
                            

                            {questions.cases && questions.cases.map((res, idx) => {
                                return (
                                    <Col xxl={4} xl={4} lg={4} md={4} sm={6} xs={12} className="">
                                        <Card border="light" bg="dark" style={{ width: '100%' }} className=" px-2 text-white ">
