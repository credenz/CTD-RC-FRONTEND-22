...
function Rdiv( props ) {
  ...
  const [compilerLang, setcompilerLang] = useState("c_cpp")
  ...
  const handleFile = (e) => {
    ...
    localStorage.setItem(`${lang}${props.qnIdParam}`,content);
  ...
  function langc(e) {
    ...
    if(e.target.value == "c"){
      setcompilerLang("c_cpp")
    }
    setUserInpText(localStorage.getItem(`${e.target.value}${props.qnIdParam}`));
  }
  function themec(e) {
  ...
  const submitInput = () =>{
    try {
      ...
      const myCodeArr =   localStorage.getItem(`${lang}${props.qnIdParam}`);
      // var codeInput = " ";
      ...
      axios(config)
        .then(function (response) {
          ...
          let a = response.data;
          // navigate(`/testcase/${props.qnIdParam}`, { state:  { cases: a.cases, error: a.error }  })
          navigate(`/testcase/${props.qnIdParam}`, { state:  a })
        })
...
function onChange(newValue) {
  ...
  localStorage.setItem(`${lang}${props.qnIdParam}`, newValue);
}
    ...
    <Card className="rdiv">
      ...
      <Card className="rtopnav bg-t c-w">
        ...
        <div className="rtn rtn2">
          ...
          <h5>Your Score :{props.userScoreProp}</h5>
        </div>
      ...
      <Card className="ediv">
        <AceEditor
          mode={compilerLang}
          value={localStorage.getItem(`${lang}${props.qnIdParam}`).length>0 ? localStorage.getItem(`${lang}${props.qnIdParam}`) : userInpText }
          onChange={onChange}
          // onLoad={this.onLoad}
          theme={theme}
